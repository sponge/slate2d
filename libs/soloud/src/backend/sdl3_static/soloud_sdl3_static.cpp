...
result sdl3static_init(SoLoud::Soloud *aSoloud, unsigned int aFlags, unsigned int aSamplerate, unsigned int aBuffer,
  ...
  gAudioStream = SDL_OpenAudioDeviceStream(
    SDL_AUDIO_DEVICE_DEFAULT_OUTPUT, &gActiveAudioSpec, soloud_sdl3static_audiomixer, (void *)aSoloud);
  if (!gAudioStream) {
    return UNKNOWN_ERROR;
  }

  aSoloud->postinit(gActiveAudioSpec.freq, gSamples, aFlags, gActiveAudioSpec.channels);
