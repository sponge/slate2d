...
class Game {
  ...
  constructor(initialState: string, mapName?: string, startPos?: [number, number]) {
    Buttons.register();
  ...
  draw() {
    ...
    Draw.setColor(255, 255, 255, 255);
    for (const ent of this.state.entities) if (ent.layer == Layer.Back) ent.draw();
    this.map.draw('BGTiles');
    this.map.draw('BGWorld');
    this.map.draw('BGDecoration');
    for (const ent of this.state.entities) if (ent.layer == Layer.Background) ent.draw();
    this.map.draw('Collision');
    for (const ent of this.state.entities) if (ent.layer == Layer.Normal) ent.draw();
    for (const ent of this.state.entities) if (ent.layer == Layer.Foreground) ent.draw();
    Draw.setColor(255, 255, 255, 255);
