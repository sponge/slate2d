...
RLAPI unsigned int rlLoadAttribBuffer(unsigned int vaoId, int shaderLoc, void *buffer, int size,
                                      bool dynamic); // Load a new attributes buffer
...
RLAPI void rlSetBlendMode(int glSrcFactor, int glDstFactor,
...
RLAPI Shader LoadShaderCode(const char *vsCode,
...
RLAPI rlRectangle GetShapesTextureRec(void); // Get texture rectangle to draw shapes
...
GenTextureCubemap(Shader shader, Texture2D panorama, int size,
                  int format); // Generate cubemap (6 faces) from equirectangular (panorama) texture
RLAPI TextureCubemap GenTextureIrradiance(Shader shader, TextureCubemap cubemap,
                                          int size); // Generate irradiance cubemap using cubemap texture
RLAPI TextureCubemap GenTexturePrefilter(Shader shader, TextureCubemap cubemap,
...
void rlBegin(int mode)
{
  ...
  if (RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1].mode != mode) {
    if (RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1].vertexCount > 0) {
      ...
      if (rlCheckBufferLimit(RLGL.currentBatch->draws[RLGL.currentBatch->drawsCounter - 1].vertexAlignment))
...
void rlDrawMesh(Mesh mesh, Material material, Matrix transform)
{
  ...
  if (material.shader.locs[SHADER_LOC_MATRIX_MODEL] != -1)
    SetShaderValueMatrix(material.shader, material.shader.locs[SHADER_LOC_MATRIX_MODEL], transform);
  ...
  if (material.shader.locs[SHADER_LOC_MATRIX_VIEW] != -1)
    SetShaderValueMatrix(material.shader, material.shader.locs[SHADER_LOC_MATRIX_VIEW], RLGL.State.modelview);
  if (material.shader.locs[SHADER_LOC_MATRIX_PROJECTION] != -1)
    SetShaderValueMatrix(material.shader, material.shader.locs[SHADER_LOC_MATRIX_PROJECTION], RLGL.State.projection);
...
void *rlReadTexturePixels(Texture2D texture)
{
  ...
  pixels =
    (unsigned char *)RL_MALLOC(GetPixelDataSize(texture.width, texture.height, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8));
...
rlRectangle GetShapesTextureRec(void)
{
#if defined(GRAPHICS_API_OPENGL_11)
  rlRectangle rec = {0};
  return rec;
...
int GetShaderLocation(Shader shader, const char *uniformName)
{
  ...
  if (location == -1)
    TRACELOG(LOG_WARNING, "SHADER: [ID %i] Failed to find shader uniform: %s", shader.id, uniformName);
  else
    TRACELOG(LOG_INFO, "SHADER: [ID %i] Shader uniform (%s) set at location: %i", shader.id, uniformName, location);
#endif
  return location;
}
...
int GetShaderLocationAttrib(Shader shader, const char *attribName)
{
  ...
  if (location == -1)
    TRACELOG(LOG_WARNING, "SHADER: [ID %i] Failed to find shader attribute: %s", shader.id, attribName);
  else
    TRACELOG(LOG_INFO, "SHADER: [ID %i] Shader attribute (%s) set at location: %i", shader.id, attribName, location);
#endif
  return location;
}
...
void SetShaderValueV(Shader shader, int locIndex, const void *value, int uniformType, int count)
{
  ...
  case SHADER_UNIFORM_SAMPLER2D:
    ...
    break;
  default:
    TRACELOG(LOG_WARNING, "SHADER: [ID %i] Failed to set uniform, data type not recognized", shader.id);
  }
...
void EndBlendMode(void) { BeginBlendMode(BLEND_ALPHA); }
