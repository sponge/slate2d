...
#define CHECKSTR(key)                                                                                                  \
  assetSettings[key].asCString();                                                                                      \
  if (!assetSettings[key].asCString())                                                                                 \
    return JS_ThrowTypeError(ctx, "Wrong or missing key: options." key " is not a string");
#define CHECKINT32(key)                                                                                                \
  assetSettings[key].asInt32();                                                                                        \
  if (!assetSettings[key].isNumber())                                                                                  \
    return JS_ThrowTypeError(ctx, "Wrong or missing key: options." key " is not an int");
...
{
  JS_SetModuleExportList(ctx, m, js_assets_funcs, countof(js_assets_funcs));
  return 0;
}
